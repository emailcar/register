				...
				if(!empty($_POST['txtConfirmEmail'])){
					...
					echo "zeshi sd";
						...
						if($x){
							...
							setcookie("selTitle",$user_news['selTitle'],time()+3600);
							setcookie("txtLanguage",$user_news['txtLanguage'],time()+3600);
							if($txtLanguage=="ch"){
								require_once('x/default.html');
							}elseif($txtLanguage=="en"){
								require_once('x/default_en.html');
							}else{
					...
					echo '<script type="text/javascript">alert("没有填写邮箱！");window.location.href="x/index.html";</script>';
			
				}
			}else{
				echo '<script type="text/javascript">alert("没有填写姓名");window.location.href="x/index.html";</script>';
			}
	}else{
